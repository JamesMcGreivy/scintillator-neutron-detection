...
# Loop through all data files and generate a histogram of light yield
...
for filename in os.listdir(DATADIR):
    if filename.endswith(".csv"):
        ...
        i = filename.find(".")
        unit = filename[0:i]
        output = plt.hist(lightYield, bins = BINS)
        data[energy+unit] = output[0]
...
# Dump to file
with open('keVtoLightYield.pickle', 'wb') as handle:
